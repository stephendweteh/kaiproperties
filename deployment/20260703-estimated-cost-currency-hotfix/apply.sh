#!/bin/bash
# Estimated Cost Currency Column Hotfix
# Usage: bash deployment/20260703-estimated-cost-currency-hotfix/apply.sh [optional-project-path]
...
MIGRATION_NAME="2026_07_03_120000_add_estimated_cost_currency_to_tickets_table.php"
...
LOG_FILE="$LOG_DIR/estimated-cost-currency-hotfix-$TIMESTAMP.log"

echo ""
echo "=========================================="
echo "  ESTIMATED COST CURRENCY HOTFIX"
echo "=========================================="
echo ""
...
{
    echo "[$(date)] Starting estimated_cost_currency hotfix"
    ...
    echo "Verifying tickets.estimated_cost_currency column..."
    HAS_COLUMN=$(php artisan tinker --execute="echo \Illuminate\Support\Facades\Schema::hasColumn('tickets', 'estimated_cost_currency') ? 'yes' : 'no';" | tr -d '\r' | tail -n 1)

    if [ "$HAS_COLUMN" != "yes" ]; then
        echo "ERROR: Column tickets.estimated_cost_currency was not created."
        exit 1
    fi
